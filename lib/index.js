...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-dlang";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-dlang list item 1",
        data: {name: "list item 1"}
      },
      {
        label: "coc-dlang list item 2",
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-dlang works!`);
  context.subscriptions.push(import_coc2.commands.registerCommand("coc-dlang.Command", async () => {
    import_coc2.window.showMessage(`coc-dlang Commands works!`);
  }), import_coc2.listManager.registerList(new lists_default(import_coc2.workspace.nvim)), import_coc2.sources.createSource({
    name: "coc-dlang completion source",
    doComplete: async () => {
      const items = await getCompletionItems();
      return items;
    }
  }), import_coc2.workspace.registerKeymap(["n"], "dlang-keymap", async () => {
